...
TcpConnection::~TcpConnection() {
    LOG_DEBUG("TcpConnection::dtor[%s] at fd=%d state=%d", name_.c_str(), channel_->fd(), state_);
}
...
//应用写的快，内核发送慢，需要把待发送数据写入buffer，而且设置了水位cb
void TcpConnection::sendInLoop(const void* message, size_t len) {
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) {
        ...
        if (nwrote >= 0) {
            ...
            if (remaining == 0 && writeCompleteCallback_) {
                //数据全部发送完成，不用再给channel设置epollout事件
                loop_->queueInLoop(std::bind(writeCompleteCallback_, shared_from_this()));
            }
        } else {
            nwrote = 0;
    ...
    //数据未全部发送出去，剩余数据需要保存到buffer中，然后给channel注册epollout
    //poller发现tcp的发送缓冲区有空间，通知相应的sock-channel，调用writecb
    //即handlewrite方法，把发送缓冲区中数据全部发送完成
    if (!faultError && remaining > 0) {
        ...
        if (!channel_->isWriting()) {
            channel_->enableWriting();  //否则poller不会给channel通知epollout
        }
    }
}
